...
def _format_doc(doc: Optional[dict]) -> Optional[dict]:
    """Converte ObjectId para string e ajusta o campo _id -> id."""
    if doc:
        doc["id"] = str(doc["_id"])
        del doc["_id"]
    return doc
...
async def create_edital(
    ...
    edital_in: EditalCreate,
    professor_id: str,
    professor_username: str
) -> dict:
    """Cria um novo edital no DB."""
    edital_data = edital_in.model_dump()
    edital_data["professor_id"] = professor_id
    edital_data["professor_username"] = professor_username
    edital_data["created_at"] = datetime.utcnow()

    result = await db.editais.insert_one(edital_data)
    return await get_edital_by_id(db, str(result.inserted_id))


async def get_multi_editais(db: AsyncIOMotorDatabase, skip: int = 0, limit: int = 100) -> List[dict]:
    """Lista TODOS os editais com paginação, ordenando por data de fim ascendente."""
    editais_cursor = db.editais.find().sort("data_fim", 1).skip(skip).limit(limit)
...
async def get_multi_editais_validos(db: AsyncIOMotorDatabase, skip: int = 0, limit: int = 100) -> List[dict]:
    """Lista apenas os editais válidos (data_fim >= agora)."""
    query = {"data_fim": {"$gte": datetime.utcnow()}}
    editais_cursor = db.editais.find(query).sort("data_fim", 1).skip(skip).limit(limit)
...
async def update_edital(db: AsyncIOMotorDatabase, id: str, edital_update: dict) -> Optional[dict]:
    """Atualiza um edital existente pelo ID."""
    try:
        oid = ObjectId(id)
    except:
        return None
    ...
    result = await db.editais.update_one(
        {"_id": oid},
        {"$set": edital_update}
    )
